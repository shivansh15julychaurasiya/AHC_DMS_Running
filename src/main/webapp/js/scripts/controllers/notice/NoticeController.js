...
EDMSApp.controller('NoticeController',['$scope','$http','$sce','Upload',function ($scope, $http,$sce,Upload) {
	...
	$scope.submitForm1 = function() {
		console.log("fsfsfsdfdff",$scope.applicationNotice);
		$http
		.post(urlBase+"notice/application_notice",JSON.stringify($scope.applicationNotice))
	...
	$scope.submitForm = function() {
		console.log($scope.caseNotice);
		$http
		.post(urlBase+"notice/case_notice",JSON.stringify($scope.caseNotice))
		.then(function(response) {
			$scope.responseData = response;
		}, function(response){
			console.log("Exception details: " + JSON.stringify(response));
